module.exports.addResource = function(options) {
  ...
  var schema       = options.table.schema;
  ...
  app.post(baseUrl, function(req, res) {
    ...
    table.create(postHash, function(error, resource) {
      if (!error) {
        res.json(resource);
      } else {
        res.json(error);
  ...
  app.put(baseUrl + '/:id', function(req, res) {
    var data = req.body;
    table.update(req.params.id, req.body, function(error, resource) {
      if (error) {
  ...
  app.delete(baseUrl + '/:id', function (req, res){
    return table.remove({_id: req.params.id}, function (error) {
      if (!error) {
        res.send('');
      } else {
        res.send(error);
